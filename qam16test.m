...
M=dlmread('D:\Hzj\WDM.vtmu_pack\Resources\..\Inputs\data\Channel2.txt');
...
lms  = optix.LinearEqualizerMIMO('Algorithm','LMS','StepSize',1e-2, 'NumTaps',3, 'ReferenceTap',2, ...
    'TapWeightsOutputPort', true, 'ErrorOutputPort', true, 'TrainingFlagInputPort', true,'InputSamplesPerSymbol',2,'AdaptWeights',false,'InitialWeightsXX',[0;1;0]);%,'InitialWeights',zeros(5,4));
...
legend('Oversampling of 8', 'Downsample to 2','Downsample to 1','original signal');
    ...
    QXr=(QXr>0)*2-1;
    QYr=(QYr>0)*2-1;
    a=[IXb(1000:end);IYb(1000:end);QXb(1000:end);QYb(1000:end)];
    a_cap=[IXr(1000:end);IYr(1000:end);QXr(1000:end);QYr(1000:end)];%Combined signal for BER computation
    
    
    BER(i) = sum(a~=a_cap)/length(a);%Bit Error Rate Computation
    if i==4
    for be=1:N
    BERc1(1,be)=sum(a(1:be)~=a_cap(1:be))/be;
    end
    end
end
