function [Ex_out, Ey_out] = MCMA(Ex_in, Ey_in, opt)
%Ex_in=rxt; Ey_in=ryt;
...
eps_y = zeros(N, NI);
exc=zeros(N, NI);%N:Input length, NI: Iteration number
eyc=zeros(N, NI);%N:Input length, NI: Iteration number
 Cx=0.5;Cy=0.5;
   [cx,cy]=deal(zeros(1,N*NI));
for it = 1:NI
    ...
    for n = 1:N
        ...
        exc(n,it)=real(vx)*(1/(2)-abs(real(vx))^2)+1i*imag(vx)*(1/(2)-abs(imag(vx))^2);
        eyc(n,it)=real(vy)*(1/(2)-abs(real(vy))^2)+1i*imag(vy)*(1/(2)-abs(imag(vx))^2);
          Cx=Cx+mu2*conj(x_out)*exc(n,it);
          Cy=Cx+mu2*conj(y_out)*eyc(n,it);
          cx(n+(it-1)*N)=Cx;cy(n+(it-1)*N)=Cy;
         % plot(atan(imag(cx)./real(cx)));hold on;plot(atan(imag(cy)./real(cy)));xlabel('Symbol number');ylabel('Angle of Phase rotators (Radian)');legend('Cx','Cy')
         % plot(abs(cx));hold on;plot(abs(cx));xlabel('Symbol number');ylabel('Amplitude of Phase rotators');legend('Cx','Cy')
...
%rx=1/2;ry=1/2;
        eps_x(n, it)=exc(n,it)*abs(Cx)/Cx;
        eps_y(n, it)=eyc(n,it)*abs(Cy)/Cy;

        if mod(n, sps) == mod(so, sps) % update filter taps every 'sps' samples at so+sps*x position
            h_xx(:,n+1) = h_xx(:,n) + mu * eps_x(n,it) * conj(x_in);
            h_xy(:,n+1) = h_xy(:,n) + mu * eps_x(n,it)  * conj(y_in);
            h_yx(:,n+1) = h_yx(:,n) + mu * eps_y(n,it)  * conj(x_in);
            h_yy(:,n+1) = h_yy(:,n) + mu * eps_y(n,it) * conj(y_in);
        else
            h_xx(:,n+1) = h_xx(:,n);
            h_xy(:,n+1) = h_xy(:,n);
            h_yx(:,n+1) = h_yx(:,n);
...
err_x = eps_x(so:sps:end, :);
err_y = eps_y(so:sps:end, :);
err_x = (abs(err_x)).^2;
err_y = (abs(err_y)).^2;
