...
%run C:/Users/default.DESKTOP-J08E99G/OneDrive - The Hong Kong Polytechnic University/FYP/optix/init
DEBUG = false; % output figures and extra information 
N=4096;%Number of bits for each arm
...
%[w,mse,optDelay]=mmse_equalizer(rcPulse,8,8);
%rc=conv(w,rcPulse);
%stem([1:length(rc)],rc)
...
%rcPulse=zeros(4,65);
%for q=1:4 
%alpha = 0:0.3:1; Nsym =8;L=8;% RC filter alpha and filter span in symbols 
%rcPulse(q,:) = raisedCosineFunction(alpha(q),L,Nsym);%;%Number of bits to transmit
%end
%fv=fvtool(rcPulse(1,:),1,rcPulse(2,:),1,rcPulse(3,:),1,rcPulse(4,:),1);legend(fv,'alpha=0','alpha=0.3','alpha=0.6','alpha=0.9')
...
dt = 1/40*10^3;%Period in pico second
...
nn=length(sx);
al=pi/3;the=0;
T=[cos(al),exp(-1i*the)*sin(al);-exp(1i*the)*sin(al),cos(al)];
for qq=1:nn
%aa=((T*[sx(qq);sy(qq)]));
%sx(qq)=aa(1);sy(qq)=aa(2);
end
[sx,sy]=sspropv(sx,sy,dt,dz,nz,0,0,betapa,betapb,0,0,'elliptical');%Call SSPROP function
[nx,ny]=deal(zeros(length(sx),length(EbN0dB)));
%BERc=zeros(1,N);
%BERc1=zeros(1,N);
%BERc2=zeros(1,N);
for i=1:length(EbN0dB)%for each SNR
    ...
    nx(:,i)=nxr + 1i*nxi;
    ny(:,i)=nyr + 1i*nyi;
    rx = sx + nx(:,i);%add noise
    ry = sy + ny(:,i);%add noise
    ...
    ryt=resample(ry,1,4);
     sxorigin=IXb+1i*QXb;
     syorigin=IYb+1i*QYb;
     [ranx,rany]=deal(zeros(1,2*N));
     for cn=2:2*N
     randx=rand(1)*3-3/2;ranx(cn)=ranx(cn-1)+randx;
     randy=rand(1)*3-3/2;rany(cn)=rany(cn-1)+randy;
     end
    %pfox =comm.PhaseFrequencyOffset('PhaseOffset',ranx(1:end));%phase noise
    %pfoy = comm.PhaseFrequencyOffset('PhaseOffset',rany(1:end));
   % rxt=pfox(rxt);
    %ryt=pfoy(ryt);
...
lms  = optix.LinearEqualizerMIMO('Algorithm','LMS','StepSize',1e-2, 'NumTaps',3, 'ReferenceTap',2, ...
    'TapWeightsOutputPort', true, 'ErrorOutputPort', true, 'TrainingFlagInputPort', true,'InputSamplesPerSymbol',2,'AdaptWeights',true)
...
xcorlen = 1000; % number of symbols used to do cross-correlation, a proper value should be searched to maxmize the corr. magnitude
...
if 0%DEBUG
  ...
  iy
  figure;
  subplot(211);plot(detmetx);title('magnitude of cross correlation in frame sync. of X pol.');
  subplot(212);plot(detmety);title('magnitude ofcross correlation in frame sync. of Y pol.');
end
refx = refx(ix-xcorlen+1:ix-xcorlen+length(rxt)/sps);
refy = refy(iy-xcorlen+1:iy-xcorlen+length(ryt)/sps);

rx_temp = rxt;%rxt/sqrt(mean(abs(rxt).^2));
ry_temp = ryt;%ryt/sqrt(mean(abs(ryt).^2));
outlen = length(refx);
...
if i==8
  ...
  figure;
  subplot(211); plot(abs(wxx(:,end)), '-o'); title('converged filter taps of X pol.');
  subplot(212); plot(abs(wyy(:,end)), '-o'); title('converged filter taps of Y pol.');
...
legend('Oversampling of 8', 'Downsample to 2','Downsample to 1','original signal');
    ...
    if doPlot==1        
        subplot(1,2,1)
        plot(IXr(1000:end),QXr(1000:end),'o');hold on
          xlim([-2,2]);ylim([-2,2]);
        title('X Polarization')
        subplot(1,2,2)
        plot(IYr(1000:end),QYr(1000:end),'o')
    ...
    QXr=(QXr>0)*2-1;
    QYr=(QYr>0)*2-1;
    aa=[IXb(1000:end);IYb(1000:end);QXb(1000:end);QYb(1000:end)];
    aa_cap=[IXr(1000:end);IYr(1000:end);QXr(1000:end);QYr(1000:end)];%Combined signal for BER computation
    
    
    BER(i) = sum(aa~=aa_cap)/length(aa);%Bit Error Rate Computation
    if i==4
    for be=1:N
    BERc1(1,be)=sum(aa(1:be)~=aa_cap(1:be))/be;
    end
    end
end
...
switch (plo)
    case 1
 plot(1:N,10*log(BERc),1:N,10*log(BERc1),1:N,10*log(BERc2),1:N,10*log(BERc3));%,1:N,10*log(BERc2),1:N,10*log(BERc3),'-')
 xlabel('Symbol number');ylabel('BER in dB');
legend('No compensation', 'CMA','LMS perid=10','LMS period=1');
xlim([0,4096])
