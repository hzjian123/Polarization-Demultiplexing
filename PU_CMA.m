function [Ex_out, Ey_out] = PU_CMA(Ex_in, Ey_in, opt)
%Ex_in=rxt;Ey_in=ryt;
    ...
    for n = 1:N
       ...
       if isempty(ref) || ~isempty(ref) && ~trmask(n)
        ...
        if mod(n, sps) == mod(so, sps) % update filter taps every 'sps' samples at so+sps*x position
            ...
            h_xx(sst:sst+p-1,n+1) = h_xx(sst:sst+p-1,n) + mu * eps_x(n,it) * x_out * conj(x_in(sst:sst+p-1));
            h_xy(sst:sst+p-1,n+1) = h_xy(sst:sst+p-1,n) + mu * eps_x(n,it) * x_out * conj(y_in(sst:sst+p-1));
            h_yx(sst:sst+p-1,n+1) = h_yx(sst:sst+p-1,n) + mu * eps_y(n,it) * y_out * conj(x_in(sst:sst+p-1));
            h_yy(sst:sst+p-1,n+1) = h_yy(sst:sst+p-1,n) + mu * eps_y(n,it) * y_out * conj(y_in(sst:sst+p-1));
            if count<2&&sst==4
           sst=4;
            else
               sst=sst+p;
