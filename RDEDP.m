function [Ex_out, Ey_out] = RDEDP(Ex_in, Ey_in, opt)
%Ex_in=rxt;Ey_in=ryt;
...
aa=1;bb=9;
        if mod(n, sps) == mod(so, sps) % update filter taps every 'sps' samples at so+sps*x position
            if count==0
            h_xx(aa:bb,n+1) = h_xx(aa:bb,n) + mu * eps_x(n,it) * x_out * conj(x_in(aa:bb));
            h_xy(aa:bb,n+1) = h_xy(aa:bb,n) + mu * eps_x(n,it) * x_out * conj(y_in(aa:bb));
            h_yx(aa:bb,n+1) = h_yx(aa:bb,n) + mu * eps_y(n,it) * y_out * conj(x_in(aa:bb));
            h_yy(aa:bb,n+1) = h_yy(aa:bb,n) + mu * eps_y(n,it) * y_out * conj(y_in(aa:bb));
            end
            count=count+1;
            if count==1
                count=0;
